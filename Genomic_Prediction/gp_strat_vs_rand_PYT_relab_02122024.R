...
write.csv(results, "Data/gp/gp_pa_strat_vs_rand.csv", row.names = FALSE)

# Make figures
fig_strat_rand_y <- ggplot(data = results %>% filter(trait == "yield"), mapping = aes(x = vs, y = overall_pa, fill = method)) + geom_bar(stat = "identity", position = "dodge") + xlab("Validation Set") + ylab("Mean Correlation") + labs(fill = "TS/VS Composition") + scale_fill_manual(values = c("#C0504D", "#1F497D"), labels = c(paste0("Randomly Sampled", "\n", "(Size Controlled)"), paste0("Stratified by", "\n", "Breeding Cycle"))) + geom_errorbar(aes(ymin = overall_pa-stdev, ymax = overall_pa+stdev), width = 0.2, position = position_dodge(0.9)) + theme_light()
fig_strat_rand_y

fig_strat_rand_ph <- ggplot(data = results %>% filter(trait == "height"), mapping = aes(x = vs, y = overall_pa, fill = method)) + geom_bar(stat = "identity", position = "dodge") + xlab("Validation Set") + ylab("Mean Correlation") + labs(fill = "TS/VS Composition") + scale_fill_manual(values = c("#C0504D", "#1F497D"), labels = c(paste0("Randomly Sampled", "\n", "(Size Controlled)"), paste0("Stratified by", "\n", "Breeding Cycle"))) + geom_errorbar(aes(ymin = overall_pa-stdev, ymax = overall_pa+stdev), width = 0.2, position = position_dodge(0.9)) + theme_light()
fig_strat_rand_ph

fig_strat_rand_sw <- ggplot(data = results %>% filter(trait == "sw"), mapping = aes(x = vs, y = overall_pa, fill = method)) + geom_bar(stat = "identity", position = "dodge") + xlab("Validation Set") + ylab("Mean Correlation") + labs(fill = "TS/VS Composition") + scale_fill_manual(values = c("#C0504D", "#1F497D"), labels = c(paste0("Randomly Sampled", "\n", "(Size Controlled)"), paste0("Stratified by", "\n", "Breeding Cycle"))) + geom_errorbar(aes(ymin = overall_pa-stdev, ymax = overall_pa+stdev), width = 0.2, position = position_dodge(0.9)) + theme_light()
fig_strat_rand_sw

fig_strat_rand_tw <- ggplot(data = results %>% filter(trait == "tw"), mapping = aes(x = vs, y = overall_pa, fill = method)) + geom_bar(stat = "identity", position = "dodge") + xlab("Validation Set") + ylab("Mean Correlation") + labs(fill = "TS/VS Composition") + scale_fill_manual(values = c("#C0504D", "#1F497D"), labels = c(paste0("Randomly Sampled", "\n", "(Size Controlled)"), paste0("Stratified by", "\n", "Breeding Cycle"))) + geom_errorbar(aes(ymin = overall_pa-stdev, ymax = overall_pa+stdev), width = 0.2, position = position_dodge(0.9)) + theme_light()
fig_strat_rand_tw

fig_strat_rand_hd <- ggplot(data = results %>% filter(trait == "hd"), mapping = aes(x = vs, y = overall_pa, fill = method)) + geom_bar(stat = "identity", position = "dodge") + xlab("Validation Set") + ylab("Mean Correlation") + labs(fill = "TS/VS Composition") + scale_fill_manual(values = c("#C0504D", "#1F497D"), labels = c(paste0("Randomly Sampled", "\n", "(Size Controlled)"), paste0("Stratified by", "\n", "Breeding Cycle"))) + geom_errorbar(aes(ymin = overall_pa-stdev, ymax = overall_pa+stdev), width = 0.2, position = position_dodge(0.9)) + theme_light()
fig_strat_rand_hd
...
plot_grid(fig_strat_rand_y + ylim(0,0.75) + labs(caption = "Yield (kg/ha)") + theme(legend.position = "none", plot.caption = element_text(hjust = 0.5, size = 14)), 
          ...
          strat_rand_legend,
          labels = c("A", "B", "C", "D", "E"))
ggsave("Figures/gp/strat_vs_rand_relab.pdf", height = 8.5, width = 11)
ggsave("Figures/gp/strat_vs_rand_relab.svg", height = 8.5, width = 11)
