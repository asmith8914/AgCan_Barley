...
gp_rel <- read.csv("Data/gp/gp_reliabilities.csv")
...
overall_res <- data.frame(rbind(t(gp_over_res), t(fm_over_res), t(comb_over_res))) %>%
  mutate(Cohort = gsub("_[a-z]*$", "", gsub("^[a-z]{2,}_", "", .$source)),
         ...
         method = case_when(.$method == "gp" ~ "Genomic Prediction", .$method == "fm" ~ "Family Means", .$method == "comb" ~ "50/50 Combination"),
...
write.csv(pair_ttest[,1:12], "Data/gp/fam_gp_comb_ttest_res.csv", row.names = FALSE)
...
plot_grid(fig_ttest_yield + labs(caption = "Yield (kg/ha)") + theme(legend.position = "none", plot.caption = element_text(hjust = 0.5, size = 14)),
          ...
          fig_ttest_legend,
          labels = c("A", "B", "C", "D", "E"))
ggsave("Figures/gp/fam_gp_comb_rel_ttest.svg", height = 8.5, width = 14)
ggsave("Figures/gp/fam_gp_comb_rel_ttest.pdf", height = 8.5, width = 14)
